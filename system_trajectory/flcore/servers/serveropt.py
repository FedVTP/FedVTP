import time
import torch
from flcore.clients.clientopt import clientOPT
from flcore.optimizers.fedoptimizer import FedadamOptimizer, FedadagradOptimizer, FedyogiOptimizer
...
class FedOPT(Server):
    def __init__(self, args, times):
        ...
        self.set_clients(args, clientOPT)
        self.epoch = 0
        self.dataset = args.dataset
        self.tau = 1e-3
        # self.global_optimizer = FedadamOptimizer(self.global_model.parameters(), lr=self.learning_rate, v0=tau**2, tau=tau,
        #                                   betas=(0.9, 0.999))
        total_sample = 0
    ...
    def train(self):
        ...
        for i in range(self.global_rounds + 1):
            s_t = time.time()
            # if i == 20:
            #     self.clients_weight = [0.8, 0.2]
            # self.selected_clients = self.select_clients()
            # self.selected_clients = copy.deepcopy(self.used_client)
            self.send_models()
            ...
            for j, client in enumerate(self.selected_clients):
                ...
                # scheduler = torch.optim.lr_scheduler.StepLR(client.optimizer, step_size=10, gamma=0.9)
                # scheduler = torch.optim.lr_scheduler.StepLR(client.optimizer, step_size=150, gamma=0.2)
                # scheduler.step()
            print(f"\n-------------Round number: {i}-------------")
            ...
            # global_optimizer = FedadamOptimizer(self.global_model.parameters(), lr=self.learning_rate, v0=0, tau=self.tau,betas=(0.9, 0.999))
            global_optimizer = FedadagradOptimizer(self.global_model.parameters(), lr=self.learning_rate, v0=0,
                                                tau=self.tau, beta=0)
            global_optimizer.add_param_group(dict(params=list(self.global_model.buffers())))  # add buffered tensors (i.e., gamma and beta of batchnorm layers)
            global_optimizer.zero_grad(set_to_none=True)
            for j, client in enumerate(self.selected_clients):
                local_layers_iterator = itertools.chain.from_iterable([client.model.named_parameters(), client.model.named_buffers()])
                global_optimizer.accumulate(self.uploaded_weights[j], local_layers_iterator)
            # print("origin")
